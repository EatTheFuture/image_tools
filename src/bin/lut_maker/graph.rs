...
pub fn graph_ui(ui: &mut Ui, app: &mut crate::AppMain) {
    ...
    // Transfer function/exposures graph.
    {
        ...
        match (ui_data.preview_mode, ui_data.mode) {
            (PreviewMode::ExposureMappings, _) => {
                // Normalized to-linear luts.
                let luts: Vec<Vec<f32>> = match ui_data.mode {
                    AppMode::Estimate => {
            ...
            // Transfer function view for estimate curve.
            (_, AppMode::Estimate) => {
            ...
            // Transfer function view for analytic curve.
            (_, AppMode::Generate) => {
                        ...
                        for chan in 0..3 {
                            if show_from_linear_graph {
                                let range_min = (0..3).fold(std::f32::INFINITY, |a, i| {
                                    a.min(
                                        function.to_linear_fc(0.0, floor[i], ceiling[i], normalize),
                                    )
                                });
                                let range_max = (0..3).fold(-std::f32::INFINITY, |a, i| {
                                    a.max(
                                        function.to_linear_fc(1.0, floor[i], ceiling[i], normalize),
                                    )
                                });
