...
impl ColorSpaceSpec {
    ...
    fn from_str(text: &str) -> Result<ColorSpaceSpec, ConfigLoadErr> {
        ...
        for line in text.lines() {
            if let Some((param, value)) = line.split_once(":") {
                ...
                match param {
                    ...
                    "transfer_lut_path" => match lib::job_helpers::load_1d_lut(value) {
                        Ok(lut) => color_space.transfer_lut = Some((lut, value.into(), false)),
                        Err(_) => return Err(ConfigLoadErr::FileUnloadable(value.into())),
                    },
